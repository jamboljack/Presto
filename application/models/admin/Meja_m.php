...
class Meja_m extends CI_Model
{
    ...
    {
        $data = array(
            'meja_nama'   => strtoupper(trim(stripHTMLtags($this->input->post('nama', 'true')))),
            'meja_update' => date('Y-m-d H:i:s'),
        );

        $this->db->insert('presto_meja', $data);
    }

    public function select_by_id($id)
    {
        $this->db->select('*');
        $this->db->from('presto_meja');
    ...
    public function update_data()
    {
        ...
        $this->db->update('presto_meja', $data);
    }

    public function delete_data($id)
    {
        $this->db->where('meja_id', $id);
        $this->db->delete('presto_meja');
    }
}
